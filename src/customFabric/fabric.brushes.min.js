...
/*(function (f) {
  ...
  f.util.colorValues = function (a) {
    if (a) {
      ...
      if ("#" === a[0])
        return (
          ...
          [
            ...
            7 < a.length ? parseInt(a.substr(7, 2), 16) / 255 : 1,
          ]
        );
      if (-1 === a.indexOf("rgb")) {
      ...
      if (0 === a.indexOf("rgb"))
        return (
          -1 === a.indexOf("rgba") && (a += ",1"),
          a.match(/[\.\d]+/g).map(function (c) {
            return +c;
          })
        );
    }
  };
  f.Point.prototype.angleBetween = function (a) {
    return Math.atan2(this.x - a.x, this.y - a.y);
  };
  ...
  f.BaseBrush.prototype.convertToImg = function () {
    ...
    this.canvas.add(b).clearContext(this.canvas.contextTop);
  ...
  f.Stroke = f.util.createClass(f.Object, {
    color: null,
    inkAmount: null,
    lineWidth: null,
    _point: null,
    ...
    initialize: function (a, b, c, e, g, h) {
      var l = f.util.getRandom(c),
        m = f.util.getRandom(2 * Math.PI),
        k = f.util.getRandom(2 * Math.PI);
      c = l * Math.sin(k);
      l = (l / 2) * Math.cos(k);
      k = Math.cos(m);
      m = Math.sin(m);
      this.ctx = a;
      this.color = e;
      this._point = new f.Point(b.x + c * k - l * m, b.y + c * m + l * k);
      this.lineWidth = g;
      this.inkAmount = h;
      this._currentLineWidth = g;
      a.lineCap = "round";
    },
    update: function (a, b, c) {
      this._lastPoint = f.util.object.clone(this._point);
      this._point = this._point.addEquals({ x: b.x, y: b.y });
      a = this.inkAmount / (c + 1);
      this._currentLineWidth = this.lineWidth * (0.3 < a ? 0.2 : 0 > a ? 0 : a);
    },
    draw: function () {
      var a = this.ctx;
      a.save();
  ...
  f.FurBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    opacity: 1,
    width: 1,
    _count: 0,
    ...
    onMouseDown: function (a) {
      ...
      var b = this.canvas.contextTop,
        c = f.util.colorValues(this.color);
      b.strokeStyle =
        "rgba(" +
        c[0] +
        "," +
    ...
    onMouseMove: function (a) {
      ...
      b.lineTo(a.x, a.y);
      b.stroke();
      for (c = 0; c < this._points.length; c++) {
        var e = this._points[c].x - this._points[this._count].x;
        var g = this._points[c].y - this._points[this._count].y;
        var h = e * e + g * g;
        2e3 > h &&
          Math.random() > h / 2e3 &&
          (b.beginPath(),
          b.moveTo(a.x + 0.5 * e, a.y + 0.5 * g),
          b.lineTo(a.x - 0.5 * e, a.y - 0.5 * g),
          b.stroke());
      }
      this._count++;
    },
    onMouseUp: function (a) {
      0 < this._count && this.convertToImg();
    },
    _render: function () {},
  });
  f.InkBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    opacity: 1,
    width: 30,
    _baseWidth: 20,
    ...
    initialize: function (a, b) {
      ...
      this.color = b.color || a.freeDrawingBrush.color;
      this.opacity = b.opacity || a.contextTop.globalAlpha;
      this._point = new f.Point();
    },
    _render: function (a) {
      var b = this.setPointer(a),
        c = b.subtract(this._lastPoint),
        e = b.distanceFrom(this._lastPoint);
      var g = 0;
      for (a = this._strokes.length; g < a; g++) {
        var h = this._strokes[g];
        h.update(b, c, e);
        h.draw();
      }
      30 < e && this.drawSplash(b, this._inkAmount);
    },
    onMouseDown: function (a) {
      this.canvas.contextTop.globalAlpha = this.opacity;
      this._resetTip(a);
    ...
    onMouseMove: function (a) {
      this.canvas._isCurrentlyDrawing && this._render(a);
    },
    onMouseUp: function () {
      this.convertToImg();
      this.canvas.contextTop.globalAlpha = 1;
    },
    drawSplash: function (a, b) {
      var c,
        e = this.canvas.contextTop,
        g = f.util.getRandom(12),
        h = 10 * b,
        l = this.color;
      e.save();
      for (c = 0; c < g; c++) {
        var m = f.util.getRandom(h, 1);
        var k = f.util.getRandom(2 * Math.PI);
        k = new f.Point(a.x + m * Math.sin(k), a.y + m * Math.cos(k));
        e.fillStyle = l;
        e.beginPath();
        e.arc(k.x, k.y, f.util.getRandom(b) / 2, 0, 2 * Math.PI, !1);
        e.fill();
      }
      e.restore();
    },
    setPointer: function (a) {
      a = new f.Point(a.x, a.y);
      this._lastPoint = f.util.object.clone(this._point);
      return (this._point = a);
    },
    _resetTip: function (a) {
      var b = this.setPointer(a);
      this._strokes = [];
      this.size = this.width / 5 + this._baseWidth;
      this._range = this.size / 2;
      var c = 0;
      for (a = this.size; c < a; c++)
        this._strokes[c] = new f.Stroke(
  ...
  f.LongfurBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    opacity: 1,
    width: 1,
    _count: 0,
    ...
    onMouseDown: function (a) {
      ...
      var b = f.util.colorValues(this.color);
      a.strokeStyle =
        "rgba(" +
        b[0] +
        "," +
    ...
    onMouseMove: function (a) {
      ...
      for (a = 0; a < this._points.length; a++) {
        var c = -Math.random();
        var e = this._points[a].x - this._points[this._count].x;
        var g = this._points[a].y - this._points[this._count].y;
        var h = e * e + g * g;
        4e3 > h &&
          Math.random() > h / 4e3 &&
          (b.beginPath(),
          b.moveTo(
            this._points[this._count].x + e * c,
            this._points[this._count].y + g * c
          ),
          b.lineTo(
            this._points[a].x - e * c + 2 * Math.random(),
            this._points[a].y - g * c + 2 * Math.random()
          ),
          b.stroke());
      }
      this._count++;
  ...
  f.MarkerBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    opacity: 1,
    width: 30,
    _baseWidth: 10,
    ...
    _render: function (a) {
      ...
      c.beginPath();
      var e = 0;
      for (b = this._size / this._lineWidth / 2; e < b; e++) {
        var g = (this._lineWidth - 1) * e;
        c.globalAlpha = 0.8 * this.opacity;
        c.moveTo(this._lastPoint.x + g, this._lastPoint.y + g);
        c.lineTo(a.x + g, a.y + g);
        c.stroke();
      }
      this._lastPoint = new f.Point(a.x, a.y);
    },
    onMouseDown: function (a) {
      this._lastPoint = a;
  ...
  f.RibbonBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    opacity: 1,
    width: 1,
    _nrPainters: 50,
    ...
    initialize: function (a, b) {
      ...
      for (a = 0; a < this._nrPainters; a++)
        this._painters.push({
          ...
          ay: 0,
          div: 0.1,
          ease: 0.2 * Math.random() + 0.6,
        });
    },
    update: function () {
      for (
        var a = this.canvas.contextTop, b = this._painters, c = 0;
        c < b.length;
        c++
      )
        a.beginPath(),
          a.moveTo(b[c].dx, b[c].dy),
          (b[c].dx -= b[c].ax =
            (b[c].ax + (b[c].dx - this._lastPoint.x) * b[c].div) * b[c].ease),
          (b[c].dy -= b[c].ay =
            (b[c].ay + (b[c].dy - this._lastPoint.y) * b[c].div) * b[c].ease),
          a.lineTo(b[c].dx, b[c].dy),
          a.stroke();
    },
    onMouseDown: function (a) {
      var b = this.canvas.contextTop,
        c = f.util.colorValues(this.color);
      this._painters = [];
      for (var e = 0; e < this._nrPainters; e++)
        this._painters.push({
          dx: this.canvas.width / 2,
          dy: this.canvas.height / 2,
          ax: 0,
      ...
      b.strokeStyle =
        ...
        ")";
      b.lineWidth = this.width;
      for (e = 0; e < this._nrPainters; e++)
        (this._painters[e].dx = a.x), (this._painters[e].dy = a.y);
      var g = this;
  ...
  f.ShadedBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    opacity: 0.1,
    width: 1,
    shadeDistance: 1e3,
    ...
    initialize: function (a, b) {
      ...
      this.width = b.width || a.freeDrawingBrush.width;
      this.color = b.color || a.freeDrawingBrush.color;
      this.opacity = b.opacity || 0.3;
      this.shadeDistance = b.shadeDistance || 1e3;
    },
    onMouseDown: function (a) {
      this._points = [a];
      a = this.canvas.contextTop;
    ...
    onMouseMove: function (a) {
      ...
      var b = this._points;
      a.beginPath();
      a.moveTo(b[b.length - 2].x, b[b.length - 2].y);
      a.lineTo(b[b.length - 1].x, b[b.length - 1].y);
      a.stroke();
      for (var c = 0, e = b.length; c < e; c++)
        (dx = b[c].x - b[b.length - 1].x),
          (dy = b[c].y - b[b.length - 1].y),
          (d = dx * dx + dy * dy),
          d < this.shadeDistance &&
            (a.beginPath(),
            a.moveTo(
              b[b.length - 1].x + 0.2 * dx,
              b[b.length - 1].y + 0.2 * dy
            ),
            a.lineTo(b[c].x - 0.2 * dx, b[c].y - 0.2 * dy),
            a.stroke());
    },
    onMouseUp: function (a) {
      1 < this._points.length && this.convertToImg();
    },
    _render: function () {},
  });
  f.SketchyBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    opacity: 1,
    width: 1,
    _count: 0,
    ...
    onMouseMove: function (a) {
      ...
      for (a = 0; a < e.length; a++) {
        ...
        4e3 > m &&
          Math.random() > m / 2e3 &&
          (c.beginPath(),
          c.moveTo(e[g].x + h * b, e[g].y + l * b),
          c.lineTo(e[a].x - h * b, e[a].y - l * b),
          c.stroke());
      }
      this._count++;
    },
  ...
  f.SpraypaintBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    opacity: 1,
    width: 30,
    _baseWidth: 40,
    ...
    initialize: function (a, b) {
      ...
      f.Image.fromURL(
        ...
        function (e) {
          c.brush = e;
          c.brush.filters = [];
          c._setColor(c.color || this.color);
        },
    ...
    _setColor: function (a) {
      this.color = a;
      this.brush.filters[0] = new f.Image.filters.BlendColor({
        color: a,
        alpha: 1,
        mode: "tint",
      });
    ...
    onMouseDown: function (a) {
      this.canvas.contextTop.globalAlpha = this.opacity;
      this._lastPoint = this._point = new f.Point(a.x, a.y);
    ...
    onMouseMove: function (a) {
      ...
      this._point = new f.Point(a.x, a.y);
    },
    onMouseUp: function () {
      var a = this;
      setTimeout(function () {
    ...
    _render: function () {
      function a() {
        var c = new f.Point(b._point.x || 0, b._point.y || 0);
        var e = c.distanceFrom(b._lastPoint);
        c = c.angleBetween(b._lastPoint);
        b._inkAmount += 100 / b.size / (Math.pow(e, 2) + 1);
        b._inkAmount = Math.max(b._inkAmount - e / 10, 0);
        b.canvas.contextTop.drawImage(
          b.brush._element,
          b._lastPoint.x + Math.sin(c) - b.size / 2,
          b._lastPoint.y + Math.cos(c) - b.size / 2,
          b.size,
          b.size
        );
        b.canvas._isCurrentlyDrawing ? setTimeout(a, b._interval) : b._reset();
      }
      var b = this;
      a();
    },
    _reset: function () {
      this._lastPoint = this._point = null;
      this.canvas.contextTop.globalAlpha = 1;
    },
  });
  f.SquaresBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    bgColor: "#fff",
    opacity: 1,
    width: 1,
    ...
    initialize: function (a, b) {
      ...
      this.width = b.width || a.freeDrawingBrush.width;
      this.color = b.color || a.freeDrawingBrush.color;
      this.bgColor = b.bgColor || "#fff";
      this.opacity = b.opacity || a.contextTop.globalAlpha;
    },
    onMouseDown: function (a) {
      var b = this.canvas.contextTop,
        c = f.util.colorValues(this.color),
        e = f.util.colorValues(this.bgColor);
      this._lastPoint = a;
      this._drawn = !1;
      this.canvas.contextTop.globalAlpha = this.opacity;
      b.fillStyle = "rgba(" + e[0] + "," + e[1] + "," + e[2] + "," + e[3] + ")";
      b.strokeStyle =
    ...
    onMouseMove: function (a) {
      var b = this.canvas.contextTop,
        c = a.x - this._lastPoint.x,
        e = a.y - this._lastPoint.y,
        g = Math.cos(1.57079633) * c - Math.sin(1.57079633) * e;
      c = Math.sin(1.57079633) * c + Math.cos(1.57079633) * e;
      b.beginPath();
      b.moveTo(this._lastPoint.x - g, this._lastPoint.y - c);
      b.lineTo(this._lastPoint.x + g, this._lastPoint.y + c);
      b.lineTo(a.x + g, a.y + c);
      b.lineTo(a.x - g, a.y - c);
      b.lineTo(this._lastPoint.x - g, this._lastPoint.y - c);
      b.fill();
      b.stroke();
  ...
  f.WebBrush = f.util.createClass(f.BaseBrush, {
    color: "#000",
    opacity: 1,
    width: 1,
    _count: 0,
    ...
    onMouseDown: function (a) {
      this._points = [a];
      this._count = 0;
      this._colorValues = f.util.colorValues(this.color);
    },
    onMouseMove: function (a) {
      this._points.push(a);
      var b = this.canvas.contextTop,
      ...
      for (a = 0; a < c.length; a++)
        (e = c[a].x - c[this._count].x),
          (g = c[a].y - c[this._count].y),
          (e = e * e + g * g),
          2500 > e &&
            0.9 < Math.random() &&
            (b.beginPath(),
            b.moveTo(c[this._count].x, c[this._count].y),
